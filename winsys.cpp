...
bool CWindow::handleEvent(int key) //przesuwanie okien
{
  switch (key)
    {
    case KEY_UP:
...
bool CInputLine::handleEvent(int c)
{
  if(CFramedWindow::handleEvent(c))  // jesli strzlka to zwroci true
    return true;
  if((c == KEY_DC) || (c == KEY_BACKSPACE)) //KEY_DC = DELETE
    {
      if (text.length() > 0)
        {
...
bool CGroup::handleEvent(int key) // przelaczenie okien
{
      ...
      if (!children.empty())
        {
          ...
          // dodanie na poczatek ostatniego elementu
          children.pop_back(); 
          // usuniecie ostatniego elementuku ktury jest teraz zdublowany i jest na koncu i na poczatku listy 
        };
      return true;
    }
  return false;
...
int CDesktop::getEvent() // pobirra znak
{
  return ngetch();
}
...
{
  update_screen();
  paint(); //rysuje kropki na całum ekranie
  refresh();
  while(1)
  {
    int c = getEvent();
    if(c == 'q' || c == 'Q')
      break;
    if(c == KEY_RESIZE || handleEvent(c)) //przekierowanie do odpowiedniego kontrolera wykonania okcji
    {
      update_screen();
      paint();
      refresh();
